...
const cartReducer = (state = initState, action) => {
  ...
  if (action.type === ADD_TO_CART) {
    ...
    const nextState = produce(state, (draf) => {
      if (draf.addedItems.find(item => (item.id === action.payload))) {
        const found = draf.addedItems.find(item => (item.id === action.payload));
        found.quantity++;
        draf.total += parseInt(found.price);
      } else {
        const found = draf.items.find(item => (item.id === action.payload));
        found.quantity++;
  ...
  function removeItem(id) {
    ...
    const nextState = produce(state, (draf) => {
      const foundId = draf.addedItems.find(item => (item.id === id));
      if (foundId) {
